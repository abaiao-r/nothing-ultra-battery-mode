...
This repo has a small set of scoped agents in `.github/agents/`:
- `code-reviewer`: read-only PR review against this repo's architecture/governance rules
- `issue-starter`: reads a GitHub issue, creates the matching local branch
- `pr-finalizer`: commits, pushes, creates/updates the PR from the template
- `agent-red` / `agent-green`: optional TDD pair (RED writes failing unit/ATP tests, GREEN writes the minimum code to pass); a PreToolUse hook enforces the test/production split
- `diagram-generator`: generates PlantUML architecture diagrams into `docs/diagrams/`
